<section>
    <form class="catDetail" action="{{route('addProd')}}" method="post">
        @csrf
        <label for="categoriesCid">{{"TempPid: #000".$product['tempPid']}}</label>
        <input type="hidden" name="temppid" value="{{$product['tempPid']}}">
        ...
        <div class="avatar">
            ...
            <input type="text" id="fileInput" class="casSearch" name="imgChanges" value="{{$product['Main_image']}}" placeholder="enter link img" style="display: none" onchange="previewImage()">
        </div>
